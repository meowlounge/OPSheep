...
public class SuperShearItem extends ShearsItem {
	...
	public ActionResult useOnEntity(ItemStack stack, PlayerEntity player, LivingEntity entity, Hand hand) {
		...
		if (!(sheep instanceof OpSheepAccessor opSheep) || !opSheep.opsheep$isOpSheep()) {
		...
		if (this.opShearVersion >= 2 && world.getRandom().nextFloat() < 0.10f) {
			...
			player.sendMessage(net.minecraft.text.Text.translatable("item.opsheep.supershear_perk_v2"), true);
		}
		...
		if (this.opShearVersion >= 4) {
			for (SheepEntity nearbySheep : world.getEntitiesByClass(
					...
					s -> s instanceof OpSheepAccessor && ((OpSheepAccessor) s).opsheep$isOpSheep() && !s.isSheared())) {
				shearSheep(nearbySheep, player, stack);

				player.sendMessage(net.minecraft.text.Text.translatable("item.opsheep.supershear_perk_v4"), true);
			}
		}
		...
		if (world instanceof ServerWorld serverWorld) {
			if (this.opShearVersion >= 3) {
				if (world.getRandom().nextFloat() < 0.25f) {
					...
					player.sendMessage(net.minecraft.text.Text.translatable("item.opsheep.supershear_perk_v3"), true);
				}
			}
			...
			if (this.opShearVersion >= 5 && world.getRandom().nextFloat() < 0.05f) {
				...
				if (treasures.length > 0 && !treasures[0].isEmpty()) {
					...
					player.sendMessage(net.minecraft.text.Text.translatable("item.opsheep.supershear_perk_v5"
